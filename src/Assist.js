...
const initialize = (getState) => {
  console.log('initialize assistant');
  ...
  if (devType === 'development') {
    return createSmartappDebugger({
      ...
      // initPhrase: `Запусти супертест два`,
...
function Assist() {
  ...
  useEffect(() => {
    assistantRef.current = initialize(() => getStateForAssistant());
    ...
    assistantRef.current.on('start', (event) => {
      let initialData = assistantRef.current.getInitialData();
